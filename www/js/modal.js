...
var modal = (function () {
	var SETTINGS = {
		...
		hideLinkSelector: '.b-close-modal, .b-modal-calculation .submit .link',
		classLinkReload: 'link-reload',
		gallery: {
			html:
	...
	function addContent(){
		...
		if(paper.length > 0){
			var calculationCost = $('.b-calculation-cost'),
					categories = $('#category'),
					insertCategories = $('.b-modal-calculation .b-general-form .content-left .label-left'),
					insertCalc = $('.b-modal-calculation .b-general-form .content-left');

			insertCategories.after(categories);
			insertCalc.after(calculationCost);
	...
	function removeContent(){
		...
		if(paper.length > 0){
			var calculationCost = $('.b-calculation-cost'),
					categories = $('#category'),
					insertCategories = $('.b-fields .item-category .content-left .label-left'),
					insertCalc = $('.b-fields .item-calculation');

			insertCategories.after(categories);
			insertCalc.append(calculationCost)
		}
	}
	...
	function prepareHtml(){
		...
		_hideLinkTrue = $(SETTINGS.hideLinkSelectorTrue);
	...
	function assignEvents(){
		...
		_hideLink.live('click', function(event){
			...
			if(!thisLink.hasClass(SETTINGS.classLinkReload))
				event.preventDefault();
		});
